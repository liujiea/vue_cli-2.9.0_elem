<template>
    <div class="food_container">
                        ...
                        <section class="category_left">
                            <ul>
                                <li v-for="(item, index) in category" :key="index" class="category_left_li" :class="{category_active: restaurant_category_id == item.id}" @click="selectCategoryName(item.id, index)">
                                    <section>
                                        <img src="" alt="" :src="getImgPath(item.image_url)" v-if="index" class="category_icon" />
                                        <span>{{item.name}}</span>
                                    </section>
                                    <section>
                        ...
                        <section style="width: 100%">
                            ...
                            <ul class="filter_ul">
                                <li v-for="item in Delivery" :key="item.id" class="filter_li" @click="selectDeliveryMode(item.id)">
                                    ...
                                    <span :class="{select_filter: delivery_mode == item.id}">{{item.text}}</span>
                                </li>
                            </ul>
                        </section>
                        <section style="width: 100%">
        ...
        <section class="shop_list_container">
            <shop-list :geohash="geohash" :restaurantCategoryId="restaurant_category_id" :restaurantCategoryIds="restaurant_category_ids" :sortByType="sortByType" :deliveryMode="delivery_mode" :confirmSelect="confirmStatus"></shop-list>
        </section>
    </div>
</template>
<script>
    ...
    export default {
        data () {
            return {
                ...
                headTitle: '',  // msiet 页面头部标题
                foodTitle: '',  //排序左侧头部标题
                restaurant_category_id: '', //食品类型id值
                restaurant_category_ids: '',  //筛选类型的id
                sortBy: '',   //筛选的条件
                category: null,  //category 分类左侧数据
                categoryDetail: null,   //Category 分类右侧数据
        ...
        methods: {
            ...
            async initData() {
                ...
                //记录support_ids的状态， 默认不选中， 点击状态取反， status位为true时为选中状态
            ...
            selectCategoryName(id, index) {
                //第一选项， 全部商家， 因为没有自己的列表， 所有点击则默认获取选所有数据
                if (index === 0) {
            ...
            //点击某个排序方式， 获取时间对象的data值， 并根据获取的值重新获取数据渲染
            sortList (event) {
                let node;
            ...
            selectDeliveryMode(id) {
                //delivery_mode 为空时， 选中当前项， 并且filterNum加一
                if (this.delivery_mode == null) {
                    this.filterNum++;
                    this.delivery_mode = id;
                    //delivery_mode 为当前已有值时， 清空所所选项， 并且filterNum减一
                } else if(this.delivery_mode == id) {
                    this.filterNum--;
                    this.delivery_mode = null;
                    //delivery_mode已有值且不等于当前选择值， 则赋值delivery_mode 为当前所选id
                } else {
                    this.delivery_mode = id;
                }
            },
