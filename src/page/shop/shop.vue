    ...
    <div>
        <section v-if="!showLoading" class="shop_container">
            ...
            <header class="shop_detail_header" ref="shopheader" :style="{zIndex: showActivities ? '14' : '10'}">
                <img src="shop.vue" alt="" :src="imgBaseUrl + shopDetailData.image_path" class="header_cover_img" />
                <section class="description_header">
                    <router-link to="/shop/shopDetail" class="description_top">
                        <section class="description_left">
                            <img src="" alt="" :src="imgBaseUrl + shopDetailData.image_path" />
                        </section>
                    ...
                    <footer class="description_footer" v-if="shopDetailData.activities.length" @click="showActivitiesFun">
                        <p class="ellipsis">
                            <span class="tip_icon" :style="{backgroundColor: '#' + shopDetailData.activities[0].icon_color, borderColor: '#' + shopDetailData.activities[0].icon_color}">{{shopDetailData.activities[0].icon_name}}
                            </span>
            ...
            <section class="change_show_type" ref="chooseType">
                <div>
                    <span :class="{activities_show: changeShowType == 'food'}" @click="changeShowType = 'food'">
                        商品
                    </span>
                </div>
                <div>
                    <span :class="{activities_show: changeShowType == 'rating'}" @click="changeShowType = 'rating'">
                        评价
                    </span>
                </div>
            </section>
                ...
                <section v-show="changeShowType == 'food'" class="food_container">
                    <section class="menu_container">
                        <section class="menu_left" id="wrapper_menu" ref="wrapperMenu">
                            <ul>
                                <li v-for="(item, index) in menuList" :key="index" class="menu_left_li" :class="{activities_menu: index==menuIndex}" @click="chooseMenu(index)">
                                    <img alt="" :src="getImgPath(item.icon_url)" v-if="item.icon_url">
                                    <span>{{item.name}}</span>
                            ...
                            <ul>
                                <li v-for="(item, index) in menuList" :key="index">
                                    <header class="menu_detail_header">
                                        ...
                                        <span class="menu_deatil_header_right" @click="showTitleDetail(index)"></span>
                                    ...
                                    <section v-for="(foods, foodindex) in item.foods" :key="foodindex" class="menu_detail_list">
                                        <router-link :to="{path: 'shop/foodDetail', query: {image_path: foods.image_path, description: foods.description, month_sales: foods.month_sales, name: foods.name, rating: foods.rating, rating_count: foods.rating_count,satisfy_rate: foods.satisfy_rate, foods, shopId}}" tag="div" class="menu_detail_link">
                                            <section class="menu_food_img">
                                                <img alt="" :src="imgBaseUrl+foods.image_path" />
                                            </section>
                        ...
                        <section>
                            ...
                            <ul class="rating_list_ul">
                                <li v-for="(item, index) in ratingList" :key="index" class="rating_list_li">
                                    <img alt="" :src="getImgPath(item.avatar)" class="user_avatar" />
                                    <section class="rating_list_details">
                                        ...
                                        <ul class="food_img_ul">
                                            <li v-for="(item, index) in item.item_ratings" :key="index">
                                                <img src="" alt="" :src="getImgPath(item.image_hash)" v-if="item.image_hash" />
                                            </li>
                                        </ul>
    ...
    export default {
        data () {
            return {
                ...
                perventRepeatRequest: false,   //防止多次触发数据请求
                ratingTagName: '',   //评论的类型
                ...
                elLeft: 0,           //当前点击加按钮在页面中的绝对top值
                elBottom: 0,         //当前点击加按钮页面中的绝对left值
        ...
        methods: {
            ...
            async initData() {
                ...
                //获取商铺信息
                this.shopDetailData = await shopDetails(this.shopId, this.latiude, this.longitude);
                //获取商铺食品列表
            ...
            /**
             * 初始化和shopCart变化时， 重新获取购物车改变过的数据， 赋值categoryNum, totalPrice, cartFoodList ，整个数据流是自上而下的形式，所有的购物车数据都交给vuex同一管理， 包括购物车组件中自身的商品数量，使整个数据流更加清晰
             */
            initCategoryNum () {
                let newArr = [];
                let cartFoodNum = 0;
         ...
         watch: {
            //showLoading 变化时说明组件已经获取初始化数据， 下一帧nextTick进行后续操作
