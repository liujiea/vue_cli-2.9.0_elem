<template>
    <div class="paddingTop search_page">
        ...
        <form class="search_form">
            <input type="search" name="search" placeholder="请输入商家或美食名称" class="search" v-model="searchValue" @input="checkInput" />
        ...
        <section v-if="restaurantList.length">
            ...
            <ul class="list_container">
                <router-link :to="{path: '/shop', query: {id: item.id}}" tag="li" v-for="item in restaurantList" :key="item.id" class="list_li">
                    <section :src="item_left">
                        <img src="" alt="" :src="imgBaseUrl+item.image_path" class="restaurant_img" />
                    </section>
    ...
    export default {
        data () {
            return {
                ...
                searchValue: '',   //搜索内容
                restaurantList: [],  //收缩返回的结果
                imgBaseUrl,    //图片域名地址
                searchHistory: [],   //搜搜历史记录
                showHistory: true,    //是否显示历史记录， 只有再返回搜索结果后隐藏
        ...
        mounted() {
            this.geohash = this.$route.params.geohahs;
            //获取搜索历史记录
