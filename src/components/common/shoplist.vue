    ...
    <div class="shoplist_container">
        <ul v-load-more="loaderMore" v-if="shopListArr.length" type="1">
            <router-link :to="{path: 'shop', query: {geohash, id: item.id}}" v-for="item in shopListArr" tag="li" :key="item.id" class="shop_li">
                <section>
                    <img src="" alt="" :src="imgBaseUrl + item.image_path" class="shop_img" />
                </section>
...
<script>
    ...
    export default {
        data () {
            return {
                offset: 0,    //批次加载店铺列表， 每次加载20个limit = 20
                shopListArr: [],  //店铺列表数据
                preventRepeatReuqest: false,  //达到底部加载数据， 防止重复数据
        ...
        methods: {
            async initData() {
                ...
                this.hideLoading();
                //开始监听scrollTop值， 达到一定程度后显示返回顶部按钮
                showBack(status => {
            ...
            async listenPropChange() {
                ...
                this.hideLoading();
                //考虑本地模拟器数据是引用类型， 所以返回一个新的数据
                this.shopListArr = [...res];
            },
            //开发环境与 编译环境loading隐藏方式不同
            hideLoading() {
        ...
        watch: {
            //监听父级传来的restaurantCategoryIds,当值发生变化的时候重新获取餐馆数据， 用于排序和筛选
            ...
            //监听父级的 确认按钮是否被点击， 并且返回一个自定义事件通知父级， 已经接收到数据， 此时父级才可以清除已选状态
